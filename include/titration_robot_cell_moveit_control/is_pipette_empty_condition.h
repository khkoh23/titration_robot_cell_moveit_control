#ifndef IS_PIPETTE_EMPTY_CONDITION_H
#define IS_PIPETTE_EMPTY_CONDITION_H
...
class IsPipetteEmptyCondition : public BT::ConditionNode {
public:
    using Pipette = titration_robot_interfaces::srv::Pipette;
    IsPipetteEmptyCondition(const std::string& name, const BT::NodeConfiguration& config, rclcpp::Node::SharedPtr node_ptr) 
    : BT::ConditionNode(name, config), node_(std::move(node_ptr)), bt_node_name_(name) {
        client_ = node_->create_client<Pipette>("pipette");
        RCLCPP_INFO(node_->get_logger(), "BT condition %s initialized. Will poll 'pipette' for holding_volume synchronously.", bt_node_name_.c_str());
    }
...
#endif // IS_PIPETTE_EMPTY_CONDITION_H