#ifndef IS_TITRATION_DONE_CONDITION_H
#define IS_TITRATION_DONE_CONDITION_H
...
class IsTitrationDoneCondition : public BT::ConditionNode {
public:
    IsTitrationDoneCondition(const std::string& name, const BT::NodeConfiguration& config, rclcpp::Node::SharedPtr node_ptr)
    : BT::ConditionNode(name, config), node_(node_ptr) {
        ...
        subscriber_ = node_->create_subscription<std_msgs::msg::Float32>("/ph", rclcpp::SensorDataQoS(), 
            std::bind(&IsTitrationDoneCondition::phCallback, this, std::placeholders::_1));
    }

    static BT::PortsList providedPorts() {
        return {
            BT::InputPort<float>("ph_target"),
            BT::InputPort<float>("ph_tolerance"),
        };
    }
    ...
    BT::NodeStatus tick() override {
        ...
        // Logic
        if (std::fabs(ph.data - ph_target_ ) > ph_tolerance_) {
            RCLCPP_INFO(node_->get_logger(), "[%s] pH value not close enough to target.", name().c_str());
            return BT::NodeStatus::FAILURE;
        }
        RCLCPP_INFO(node_->get_logger(), "[%s] pH value within tolerance of target", name().c_str());
...
private:
    ...
    float ph_target_{7.00};
    float ph_tolerance_{0.20};
...
#endif // IS_TITRATION_DONE_CONDITION_H